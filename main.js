...
var Ollama2 = class extends import_llms.LLM {
  static lc_name() {
    return "Ollama";
  }
  constructor(fields) {
  ...
  async *_streamResponseChunks(prompt, options, runManager) {
    ...
    for await (const chunk of stream) {
      ...
      if (!chunk.done) {
        yield new import_outputs2.GenerationChunk({
          text: chunk.response,
          generationInfo: {
            ...chunk,
            response: void 0
...
var TagGenerator = class {
  ...
  async isOllamaServerRunning() {
    try {
      const response = await (0, import_node_fetch.default)(`${this.baseUrl}/api/version`);
      return response.ok;
    } catch (error) {
  ...
  async suggestTags(content, existingTags = /* @__PURE__ */ new Set(), signal) {
    ...
    try {
      const messages = [
        new import_messages4.SystemMessage(
...
Generate tags:`
      ...
      return response.split(",").map((tag) => tag.trim().toLowerCase()).filter((tag) => tag.length > 0).map((tag) => tag.startsWith("#") ? tag : `#${tag}`);
    } catch (error) {
